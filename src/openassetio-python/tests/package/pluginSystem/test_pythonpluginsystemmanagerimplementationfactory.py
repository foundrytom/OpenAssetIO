...
class Test_PythonPluginSystemManagerImplementationFactory:
    ...
    def test_exposes_entry_point_name_with_expected_value(self):
        assert (
            PythonPluginSystemManagerImplementationFactory.kPackageEntryPointName
...
class Test_PythonPluginSystemManagerImplementationFactory_init:
    def test_when_no_args_or_env_vars_then_entry_point_plugins_loaded(
        ...
        a_default_configured_factory,
    ):
        assert a_default_configured_factory.identifiers() == [
            entry_point_plugin_identifier,
        ]
    ...
    def test_when_no_args_and_entry_points_disabled_env_then_entry_point_not_loaded(
        self, prepended_sys_path_with_entry_point_plugin, a_default_configured_factory, monkeypatch
    ):
        monkeypatch.setenv(
...
@pytest.fixture
def a_default_configured_factory():
    # Use a console logger so we can debug output on failure
    return PythonPluginSystemManagerImplementationFactory(ConsoleLogger())
