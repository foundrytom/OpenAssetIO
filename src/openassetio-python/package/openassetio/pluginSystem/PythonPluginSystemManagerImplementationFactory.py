#
#   Copyright 2013-2020 The Foundry Visionmongers Ltd
#
...
class PythonPluginSystemManagerImplementationFactory(ManagerImplementationFactoryInterface):
    ...
    The factory loads plugins from exposed `openassetio.manager_plugin`
    entry points, as well as additional plugins found under paths
    specified in the `OPENASSETIO_PLUGIN_PATH` env var.
    ...
    independent of **PYTHONPATH** and doesn't not need to be a subset of
    ...
    ## discovered plugins.
    kPackageEntryPointName = "openassetio.manager_plugin"
    ...
    def __scan(self):
        ...
        if self.__disableEntryPointsPlugins is None:
            self.__disableEntryPointsPlugins = os.environ.get(
                self.kDisableEntryPointsEnvVar, False
            )
        ...
        if self.__disableEntryPointsPlugins:
            ...
            self.__pluginManager.scan_entry_points(self.kPackageEntryPointName)
